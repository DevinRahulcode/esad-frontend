...
const RejectedLeave = () => {
  ...
  const fetchRejectedLeaves = async () => {
    try {
      ...
      // 3️⃣ Filter only REJECTED leaves and sort by latest
      const rejectedLeaves = response.data
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      <View style={styles.headerContent}>
        <MaterialCommunityIcons name="calendar-remove" size={32} color="#EF4444" />
        <View style={styles.headerText}>
          <Text style={styles.title}>Rejected Leave</Text>
          <Text style={styles.subtitle}>
            {leaves.length} {leaves.length === 1 ? 'request' : 'requests'} rejected
          </Text>
        </View>
      </View>
    </View>
  ...
  // Render each rejected leave item
  const renderItem = ({ item }) => (
    <View style={styles.leaveItem}>
      ...
      <View style={styles.rejectionInfo}>
        ...
        <Text style={styles.rejectionText}>Request was not approved by HR</Text>
      </View>
      
      <View style={styles.actionHint}>
        <MaterialCommunityIcons name="lightbulb-outline" size={16} color={COLORS.textMuted} />
        <Text style={styles.hintText}>You can submit a new request with revised details</Text>
      </View>
    </View>
  );
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading rejected leaves...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      <MaterialCommunityIcons name="calendar-check" size={64} color={COLORS.success} />
      <Text style={styles.emptyTitle}>Great News!</Text>
      <Text style={styles.emptySubtitle}>
        You don't have any rejected leave requests.{"\n"}
        All your requests have been approved or are pending review.
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  // Empty State Styles (positive message for no rejections)
  emptyContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
